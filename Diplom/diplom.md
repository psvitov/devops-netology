...
Так как планируется создавать произвольное количество инстансов для кластера, шаблоны файлов `inventory.tf` для создания файла инвентаризации `Ansible` и `k8s.tf` для создания файла инвентаризации для `Kuberspray` будем создавать с помошью bash-скрипта:
